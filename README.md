...
**act-gram は [Ollama](https://ollama.com/) と組み合わせることで、すべてのAI処理をPC内で完結させることができます。**

業務の自動化スクリプト生成・画面解析などを行う際、**スクリーンショットや操作内容が外部サーバーに一切送信されません**。
社内システム・個人情報を扱う環境でも、情報漏洩のリスクなく AI 支援の恩恵を受けられます。
...
> **セットアップ**: [Ollama](https://ollama.com/) をインストールし、使用したいモデル（例: `ollama pull qwen2.5-coder`）を取得するだけです。act-gram が起動中のモデルを自動検出します。APIキーの登録は不要です。
...
### AI スクリプト開発（ジェネレータ）
画面キャプチャとアクティブウィンドウ名をコンテキストとして付与し、日本語の指示から UWSCR スクリプトを生成します。生成されたコードはその場で編集・保存・即実行が可能です。

![AIスクリプト開発画面](img/script_dev.png)
