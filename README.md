...
### 操作レコーダー
記録を開始し、自動化したい操作を行います。記録を停止するとそれまでに実行した操作が分析され、マニュアル作成に記録データが引き継がれます。
...
### マニュアル作成
操作手順をステップごとに入力すると、スクリーンショット・音声ガイドと、手順を自動実行する `.uws` スクリプトをまとめてパッケージ生成します。ここで作成した内容はマルチステップ開発へデータを引き継いで編集することができます。（操作レコーダーで記録された内容は自動でこの機能に引き継がれます。）
...
### インタラクティブマニュアル生成
操作手順をステップごとに入力すると、スクリーンショット・音声ガイド付きのマニュアルと、手順を自動実行する `.uws` スクリプトを生成します。
...
**ポータブル**: `uwscr.exe` と同じフォルダに置くだけで動作します。環境変数PATHの変更は不要です。
...
- Windows 10 / 11
- [UWSCR](https://github.com/stuncloud/UWSCR/releases) (`uwscr.exe` を `act_gram.exe` と同じフォルダに配置)
